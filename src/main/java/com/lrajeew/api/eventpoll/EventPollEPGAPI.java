...
public class EventPollEPGAPI extends BaseAPI {
	private static EventPollEPGAPI apiCall;

	private EventPollEPGAPI() {
	}

	public static EventPollEPGAPI getInstance() {
		if (apiCall == null) {
			apiCall = new EventPollEPGAPI();
		}
		return apiCall;
	}

	private static Logger LOGGER = Logger.getLogger(EventPollEPGAPI.class);
	...
	public ClientResponse query(AuthenticationVO authData,
		...
		String apiEndpoint = ApiConsatants.VIVA_API + "events/vivacon/polls/19?"+ "rep="+responseType.getType();
