...
public class EventPollEPGCAPI extends BaseAPI {
	private static EventPollEPGCAPI apiCall;

	private EventPollEPGCAPI() {
	}

	public static EventPollEPGCAPI getInstance() {
		if (apiCall == null) {
			apiCall = new EventPollEPGCAPI();
		}
		return apiCall;
	}

	private static Logger LOGGER = Logger.getLogger(EventPollEPGCAPI.class);
	
	@Info(url="/events/<eventKey>/polls/<pollId>,<pollId>,…")
	public ClientResponse query(AuthenticationVO authData,
		...
		String apiEndpoint = ApiConsatants.VIVA_API + "events/vivacon/polls/19,18,?"+ "rep="+responseType.getType();
