...
public class AttendeeSessionAPI extends BaseAPI {
	private static AttendeeSessionAPI apiCall;

	private AttendeeSessionAPI() {
	}

	public static AttendeeSessionAPI getInstance() {
		if (apiCall == null) {
			apiCall = new AttendeeSessionAPI();
		}
		return apiCall;
	}
	...
	public ClientResponse query(AuthenticationVO authData,
		...
		String apiEndpoint = "http://dev.mob.vivastream.com/v2/events/vivacon/attendees/1/sessions?search=&blockFrom=&blockSize=&"+ "rep="+responseType.getType();
