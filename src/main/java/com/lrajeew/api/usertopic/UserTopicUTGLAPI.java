...
public class UserTopicUTGLAPI extends BaseAPI {
	private static UserTopicUTGLAPI apiCall;

	private UserTopicUTGLAPI() {
	}

	public static UserTopicUTGLAPI getInstance() {
		if (apiCall == null) {
			apiCall = new UserTopicUTGLAPI();
		}
		return apiCall;
	}

	private static Logger LOGGER = Logger.getLogger(UserTopicUTGLAPI.class);
	
	@Info(url="/users/<userId>/topics")
