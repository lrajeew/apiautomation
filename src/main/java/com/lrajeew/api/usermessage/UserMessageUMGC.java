...
public class UserMessageUMGC extends BaseAPI {
	private static UserMessageUMGC apiCall;

	private UserMessageUMGC() {
	}

	public static UserMessageUMGC getInstance() {
		if (apiCall == null) {
			apiCall = new UserMessageUMGC();
		}
		return apiCall;
	}

	private static Logger LOGGER = Logger.getLogger(UserMessageUMGC.class);
	
	@Info(url="/users/<userId>/messages/<messageId>")
	public ClientResponse query(AuthenticationVO authData,
		...
		String apiEndpoint = ApiConsatants.VIVA_API + "users/47593/messages/226,,227?"+ "rep="+responseType.getType();
