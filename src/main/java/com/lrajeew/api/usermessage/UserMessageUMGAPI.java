...
public class UserMessageUMGAPI extends BaseAPI {
	private static UserMessageUMGAPI apiCall;

	private UserMessageUMGAPI() {
	}

	public static UserMessageUMGAPI getInstance() {
		if (apiCall == null) {
			apiCall = new UserMessageUMGAPI();
		}
		return apiCall;
	}

	private static Logger LOGGER = Logger.getLogger(UserMessageUMGAPI.class);
	
	@Info(url="/users/<userId>/messages/<messageId>,<messageId>,�")
	public ClientResponse query(AuthenticationVO authData,
		...
		String apiEndpoint = ApiConsatants.VIVA_API + "users/47593/messages/226?"+ "rep="+responseType.getType();
