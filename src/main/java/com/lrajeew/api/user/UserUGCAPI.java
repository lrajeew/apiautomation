...
public class UserUGCAPI extends BaseAPI {
	private static UserUGCAPI apiCall;

	private UserUGCAPI() {
	}

	public static UserUGCAPI getInstance() {
		if (apiCall == null) {
			apiCall = new UserUGCAPI();
		}
		return apiCall;
	}

	private static Logger LOGGER = Logger.getLogger(UserUGCAPI.class);
	
	@Info(url="/users/<userId>,<userId>,�")
	public ClientResponse query(AuthenticationVO authData,
		...
		String apiEndpoint = ApiConsatants.VIVA_API + "users/47593,2?"+ "rep="+responseType.getType();
