...
public class SessionSPUAPI extends BaseAPI {
	private static SessionSPUAPI apiCall;

	private SessionSPUAPI() {
	}

	public static SessionSPUAPI getInstance() {
		if (apiCall == null) {
			apiCall = new SessionSPUAPI();
		}
		return apiCall;
	}

	private static Logger LOGGER = Logger.getLogger(SessionSPUAPI.class);
	...
	public ClientResponse query(AuthenticationVO authData,
		...
		String apiEndpoint = ApiConsatants.VIVA_API + "events/vivacon/sessions/beer?"+ "rep="+responseType.getType();
