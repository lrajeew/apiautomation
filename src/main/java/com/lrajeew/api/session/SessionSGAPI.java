...
public class SessionSGAPI extends BaseAPI {
	private static SessionSGAPI apiCall;

	private SessionSGAPI() {
	}

	public static SessionSGAPI getInstance() {
		if (apiCall == null) {
			apiCall = new SessionSGAPI();
		}
		return apiCall;
	}

	private static Logger LOGGER = Logger.getLogger(SessionSGAPI.class);
	...
	public ClientResponse query(AuthenticationVO authData,
		...
		String apiEndpoint = ApiConsatants.VIVA_API + "events/vivacon/sessions/foreign-chars?"+ "rep="+responseType.getType();
